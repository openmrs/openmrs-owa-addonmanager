...
describe('<ManageApps />', () => {
    ...
    it('Should render a heading', () => {
        ...
        expect(renderedComponent.find("h3").getNode().props.children).to.equal('Addon Manager');
    });

    it('Should render 5 table head titles', () => {
        expect(renderedComponent.find("th").getNodes()[0].props.children).to.equal('Logo');
        expect(renderedComponent.find("th").getNodes()[1].props.children).to.equal('Name');
        expect(renderedComponent.find("th").getNodes()[2].props.children).to.equal('Developer');
        expect(renderedComponent.find("th").getNodes()[3].props.children).to.equal('Version');
        expect(renderedComponent.find("th").getNodes()[4].props.children).to.equal('Delete');
    });
